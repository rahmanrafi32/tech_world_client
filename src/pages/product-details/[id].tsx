...
const ProductDetails = ({productsById}:any) => {
    return (
        <div className={'container mx-auto h-full md:h-screen w-screen flex justify-center mt-28'}>
            <div className={'grid grid-cols-1 md:grid-cols-2 gap-5'}>
                <div className={''}>
                    <Image src={productsById.image} alt={productsById.productName} width={500} height={500}/>
                </div>
                <div className={'flex flex-col p-4 gap-2'}>
                    <h1 className={'text-3xl'}>{productsById.productName}</h1>
                    <h2 className={'text-2xl mt-8'}>Key Features</h2>
                    <h2 className={'text-xl mt-5'}>Model: PRO MP223</h2>
                    ...
                    <button className={'btn btn-primary mt-20 mb-10'}>Buy Now</button>
                </div>
            </div>
        </div>
    );
